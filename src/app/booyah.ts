...
function loadFixedAssets() {
  ...
  const pixiLoaderResources = [].concat(
    GRAPHICAL_ASSETS,
    Object.values(rootConfig.directives.graphics),
  ...
  const jsonLoaderPromises = rootConfig.directives.jsonAssets.map(
    (jsonAssetDescription: any) => {
      if (typeof jsonAssetDescription === "string") {
        ...
        typeof jsonAssetDescription === "object" &&
  ...
  const musicLoadPromises = rootConfig.musicAudio.map(
  ...
  fixedAudioLoaderPromises.forEach((p) =>
    p.then(() => {
...
export function go(directives: Partial<Directives> = {}) {
  ...
  rootConfig.preloader = makePreloader(
    [rootConfig.directives.splashScreen, rootConfig.directives.gameLogo].filter(
      (e) => !!e
    )
  )
